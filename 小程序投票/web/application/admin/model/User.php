...
class User
{
     ...
     * @param $roleId
     */
    public function doAdd($userName, $userAccount, $userPassword, $roleId, $userEmail)
    {
        if (account_exist($userAccount)) {
            exception('帐号已存在');
        }
        $result = db('admin_user')->insert([
            'user_name' => $userName,
     ...
     * @param $roleId
     */
    public function doModify($userId, $userName, $userAccount, $userPassword, $roleId, $userEmail)
    {
        if (account_exist($userAccount)) {
            exception('帐号已存在');
        }
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public function getWeixin($pageNum, $numPerPage, $nick_name)
    {
        $list = db('wx_user')
            ->where(function ($query) use ($nick_name) {
                if (!empty($nick_name)) {
                    $query->whereOr('nick_name', 'like', "%{$nick_name}%");
                }
            })
            ->page($pageNum, $numPerPage)
            ->order('create_time desc')
     ...
     * @return int|string
     */
    public function getWeixinCount($nick_name)
    {
        $count = db('wx_user')
            ->where(function ($query) use ($nick_name) {
                if (!empty($nick_name)) {
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public function getUserAward($pageNum, $numPerPage, $openid)
    {
        $list = db('user_award')
            ->where(function ($query) use ($openid) {
                if (!empty($openid)) {
                    $query->whereOr('openid', 'like', "%{$openid}%");
                }
            })
            ->page($pageNum, $numPerPage)
            ->select();
     ...
     * @return int|string
     */
    public function getUserAwardCount($openid)
    {
        $count = db('user_award')
            ->where(function ($query) use ($openid) {
                if (!empty($openid)) {
