...
try {
    ...
    {
        $order_sn      = CONFIG['ORDER_PREFIX'] . 'RED_' . $activity_item['order_prefix'] . date('YmdHis') . mt_rand();
    } else {
        $order_sn      = CONFIG['ORDER_PREFIX'] . 'RED_' . date('YmdHis') . mt_rand();
    }
    //生成订单
    $order_status  = 0;
    ...
    //添加红包订单表
    $sql = "INSERT INTO `wangluo_red_packet` SET `activity_id` = ?,`sign_id` = ?,`openid` = ?, `amount` = ?,
 `order_sn` = ?, `order_status` = ? ,`create_time` = ? ,`update_time` = ?";
    ...
    $data = pay($order_sn, $amount, CONFIG['RED_NOTIFY_URL'], $openid, $activity_id);
    echo json_encode([
        'result' => 0,
        'data'   => $data,
    ]);
